...
import { Truck, RefreshCw, Info, Edit2, Trash2, Plus } from 'lucide-react';
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{editingId ? 'Edit Supplier' : 'Add New Supplier'}</DialogTitle>
            <DialogDescription>
              {editingId ? 'Update supplier information' : 'Create a new supplier account'}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
            ...
            <div>
              ...
              <Input
                type="number"
                value={formData.default_lead_time_days || 7}
                onChange={(e) => setFormData({ ...formData, default_lead_time_days: Number(e.target.value) })}
                min={1}
                className="mt-1"
              />
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setFormOpen(false)}>Cancel</Button>
            <Button onClick={handleSave}>{editingId ? 'Update Supplier' : 'Add Supplier'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
